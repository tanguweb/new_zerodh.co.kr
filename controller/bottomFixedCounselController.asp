...
<%
	'########################################################################################
	'#	File		: /controller/bottomFixedCounselController.asp
	'#  Create		: 
	'#	Info		: 
	'#	Update		: 
	'#	Update Memo	: 
	...
	Dim NM_FIELD_1  : NM_FIELD_1 = getText(ReqF("bottomFixedCounselUserName"))
	Dim NM_FIELD_2  : NM_FIELD_2 = getText(ReqF("bottomFixedCounselPhone"))
	Dim NM_FIELD_4  : NM_FIELD_4 = "9999"	'"빠른 상담"	'getText(UploadForm("counselType"))
	Dim NM_FIELD_7  : NM_FIELD_7 = "ulsantops!!"	'getText(UploadForm("password"))
	...
	Dim HTTP_REFERER : HTTP_REFERER = ReqF("HTTP_REFERER_bottomFixedCounsel")
	Dim HTTP_USER_AGENT : HTTP_USER_AGENT = ReqF("HTTP_USER_AGENT_bottomFixedCounsel")
	Dim HTTP_DEVICE_TYPE : HTTP_DEVICE_TYPE = ReqF("HTTP_DEVICE_TYPE_bottomFixedCounsel")
	Dim HTTP_MEDIA_TYPE : HTTP_MEDIA_TYPE = ReqF("HTTP_MEDIA_TYPE_bottomFixedCounsel")
	...
	If Result1 = "SUCCESS" Then 
		...
		Dim isSendMail : isSendMail = False 	'메일 발송할 것인지 여부
		...
		If isSendMail = True Then 
			...
			If sTo <> "" Then 
				...
				objMail.Subject = "[서울탑치과] 빠른 상담 신청이 접수되었습니다." & " (신청자: " & NM_FIELD_1 & ")"
		...
		Dim isSendSMS : isSendSMS = True 	'SMS 발송할 것인지 여부
		...
		If isSendSMS = True Then
			...
			If IsArray(aryRecvSMSList) Then 
				For sms_i = 0 To UBound(aryRecvSMSList, 2)
					jsonResult("sms_result") = sendSMS_cafe24("[서울탑치과] 신규 빠른상담이 등록되었습니다.(신청자:"&NM_FIELD_1&", 연락처:"&NM_FIELD_2&")", aryRecvSMSList(5, sms_i), "")
				Next 
			End If
		End If
