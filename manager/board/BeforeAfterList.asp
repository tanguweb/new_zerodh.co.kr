...
End If
		...
		SQL = SQL & "		, BAIMG.CD_BEFOREAFTER_ID, BAIMG.BEFORE_IMG, BAIMG.AFTER_IMG	"
		'							23				24				'25
		
		SQL = SQL & "		,ROW_NUMBER() OVER (ORDER BY BD.NO_VIEWORDER ASC, BD.CD_BOARDKEY ASC, BD.NO_BOARD_SEQ ASC) AS [NoDesc]	"	'26
...
<body class="skin-2">	
							...
							<div class="ibox-content">
								...
								<div class="table-responsive">
									...
									<table class="table table-hover">
										...
										%>
											<tr>
												<td class="text-center"><%=aryBoardList(26,iloop)%><%'=aryBoardList(1,iloop)%></td>
												<td class="text-center">
													<img src="/file/board/<%=aryBoardList(24,iloop)%>" style="max-width: 100px; max-height: 50px" />
													<img src="/file/board/<%=aryBoardList(25,iloop)%>" style="max-width: 100px; max-height: 50px" />
												</td>
												<td class="text-left">
										...
										<tfoot>
											<tr>
												<td class="text-center" colspan="<%=spancnt%>">
													<%
														Call PageNavigation("/manager/board/BeforeAfterList.asp","CD_BOARDCD="&CD_BOARDCD&"&sGUBUN="&sGUBUN&"&sKEYWORD="&sKEYWORD&"&sDISPLAY="&sDISPLAY&"&sSTARTDATE="&sSTARTDATE&"&sENDDATE="&sENDDATE, tn, pgcount, startpage, page, setsize, prevpage, nextpage)
													%>
												</td>
											</tr>
										</tfoot>
	...
	<script type="text/javascript">
		...
		function fn_Search()
		{
			...
			document.location.href = "BeforeAfterList.asp?CD_BOARDCD=<%=CD_BOARDCD%>&sGUBUN="+sgubun+"&sKEYWORD="+skeyword+"&sDISPLAY="+sDisplay+"&sSTARTDATE="+sStartdate+"&sENDDATE="+sEnddate;
		}

		function fn_boardview(CD_BOARDID)
		{
			document.location.href="BeforeAfterView.asp?page=<%=page%>&CD_BOARDID="+CD_BOARDID;
		}

		function fn_BoardIns(CD_BOARDCD,CD_BOARDID,CD_BOARDKEY,NO_BOARD_DEPTH,NO_BOARD_SEQ)
		{
			document.location.href="BeforeAfterIns.asp?CD_BOARDCD="+CD_BOARDCD+"&CD_BOARDID="+CD_BOARDID+"&CD_BOARDKEY="+CD_BOARDKEY+"&NO_BOARD_DEPTH="+NO_BOARD_DEPTH+"&NO_BOARD_SEQ="+NO_BOARD_SEQ;
		}
