...
'----------------------------------------------------------------------------------------
' [사용방법]
'response.write fn_AESEnc("11111") & "<br>"
'response.write fn_AESDec(fn_AESEnc("11111")) & "<br>"
'----------------------------------------------------------------------------------------
...
'*** Password *** 	
	...
	'ReDim hexPassword(lLength-1)
    ReDim bytPassword(lLength-1)
	
'	'*** Convert Hex to bytes ***	
'	response.write "Password in HEX<br>"
'    For lCount = 1 To lLength
'        hexPassword(lCount-1) = Hex(AscB(Mid(sPassword,lCount,1)))
'	
'		response.write AscB(Mid(sPassword,lCount,1)) & "=" & hexPassword(lCount-1) & "<br> "
'
'    Next
	
	'*** Convert to bytes ***	
'	response.write "Password in Bytes<br>"
    For lCount = 1 To lLength
        bytPassword(lCount-1) = AscB(Mid(sPassword,lCount,1))'hexPassword(lCount-1)
	
'		response.write "" & bytPassword(lCount-1) & " "
...
Function BAK_AESEncrypt(sPlain, sPassword)
    Dim bytIn()
    Dim bytOut
    Dim bytPassword()
    Dim lCount
    ...
    For lCount = 1 To lLength
        bytPassword(lCount-1) = CByte(AscB(Mid(sPassword,lCount,1)))
		response.write "" & Hex( bytPassword(lCount-1)) & " "
...
Function AESDecrypt(sCypher, sPassword)
	...
	'Send in the hex password directly ... MD 8.24.09
    'bytOut = DecryptData(bytIn, sPassword)
...
Function fn_AESEnc(str)
	fn_AESEnc = AESEncrypt(str, m_AESkey)
End Function

Function fn_AESDec(str)
	fn_AESDec = AESDecrypt(str,m_AESkey)
End Function
